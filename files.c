...
#include "Pergunta.h"
    ...
    void readFile(char *nome, int tamArrayPergunta, Pergunta *perguntas, int *contador){    
    ...
    if (!ap_ficheiro){ // Se o apontador devolvido for inválido ocorreu um erro na leitura
        ...
        *contador = fread(perguntas, sizeof (Pergunta), tamArrayPergunta, ap_ficheiro); // Lê a informação do ficheiro e armazena no array
    ...
    void writeFile(char *nome, Pergunta *perguntas, int contador){     
